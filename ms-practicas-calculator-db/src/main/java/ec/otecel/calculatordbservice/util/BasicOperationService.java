...
public abstract class BasicOperationService<R, P> {
	...
	protected BasicOperationService(HeaderInType headers, R response, P request) {
		this.headerIn = headers;
	...
	public R run() throws ComponentException {
		...
		try {
			...
			loggerService.logApp(uUidTransaction, headerIn.getExecId(), uUidTransaction,
					LoggerAppType.DSO_MCI_HEADERS_AUDIT_REQUEST.toString(), headerIn.toString(), className);

			// Registro Peticion MCI_BODY_AUDIT_REQUEST
			loggerService.logApp(uUidTransaction, headerIn.getExecId(), uUidTransaction,
					LoggerAppType.DSO_MCI_BODY_AUDIT_REQUEST.toString(), request.toString(), className);
			...
			loggerService.logApp(uUidTransaction, headerIn.getExecId(), uUidTransaction,
					LoggerAppType.DSO_MPI_BODY_AUDIT_RESPONSE.toString(), response.toString(), className);
			...
			loggerService.logApp(uUidTransaction, headerIn.getExecId(), uUidTransaction,
					LoggerAppType.DSO_AUDIT_BODY_ERROR_HANDLER.toString(), error.toString(), className);
			...
			error = new ComponentException(e.getMessage(), e, ErrorCodeType.ERROR_INESPERADO, true,
					...
					ErrorUtil.createErrorLocation(className, LayerType.ORCHESTRATION));
			...
			LoggerUtil.createExecutionLog(request.toString(), execRuntimeInfo,
					...
					new ExecutionInfoToLogging(serviceName, operationName, LayerType.ORCHESTRATION.value(),
							uUidTransaction),
					loggerService);
		}
		return response;
	...
	public BasicOperationService<R, P> setCommonInfo(String serviceName, String operationName, String className) {
	...
	public BasicOperationService<R, P> initializer(LoggerService loggerService) {
